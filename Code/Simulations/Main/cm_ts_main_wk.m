...
%}
function [dat1] = cm_ts_main_wk()%subj1,subj)
...
vL = struct;
%wkdat = [0.5,0.4,0.3,0.2,0.1,0.05,0.04,0.03,0.02,0.01,0.005];

for NTrials = 600%[100,250,500,600,700,800,900,1000]
    
    for dst = ["noalgorithm","UCB","greedy","egreedy","UCBbay","bernoulli","Poisson","Normal","bothNormal"]
        ...
        for test = 1
            ...
            for m1 = 1:11% 
                ...
                for model = [2,4,6,8]
                    clearvars -except m1 NTrials snr dat1 dst model data1 seqcon conv3 cnt2 correctAns SDMall RDU divisor cnt prb test seq prb2
                    vL.techni = 1;
                    ...
                    disp(strcat(dst,'-',num2str(m1),'-',num2str(model),'-Started'));
                    % initialization
                    [vL] = cm_ts_setupdata_wk(vL);
                    gentype = strcat("D:\Sumedh\Projects\Methods for psychiatric DBS programming\Data\Generator models\Generator Wk\","wk",num2str(m1),"\");
                    [expV] = ts_function_wk(vL,gentype);
                ...
                save(strcat('D:\Sumedh\Projects\Methods for psychiatric DBS programming\DataA\Effect of gen noise\10 BS\',dst,num2str(m1),'with10BS.mat'),'dat1','-v7.3');
                dat1 = [];
                data1 = [];
            end
            %         seq{test,:} = prb;
