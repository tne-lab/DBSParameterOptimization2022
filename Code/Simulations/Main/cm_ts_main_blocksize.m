...
%}
function [dat1] = cm_ts_main_blocksize()%subj1,subj)
...
for NTrials = 600%[100,250,500,600,700,800,900,1000]
    ...
    for dst = ["noalgorithm","greedy","egreedy","UCB","UCBbay","bernoulli","Poisson","Normal","bothNormal"]
        ...
        for test = 1
            ...
            save(strcat('D:\Sumedh\Projects\Methods for psychiatric DBS programming\DataA\Blocksize\blocksize gen uniformity for simulation\Block_',dst,num2str(model),'fdel.mat'),'dat1','-v7.3');
            dat1 = [];
            data1 = [];
            %         seq{test,:} = prb;
            %         seq_al{test,:} = prb2;
