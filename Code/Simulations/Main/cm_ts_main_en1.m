...
%}
function [dat1] = cm_ts_main_en1(totaldata)%subj1,subj)
    ...
    for dst = ["noalgorithm","UCB"]%,"UCB"]%"greedy","egreedy","UCB","UCBbay","bernoulli","Poisson","Normal","bothNormal"]
        ...
        for test = 1
            ...
            for divisor = 15
                ...
                for model = 2:2:8
                    clearvars -except totaldata NTrials snr dat1 dst model data1 seqcon conv3 cnt2 correctAns SDMall RDU divisor cnt prb test seq prb2
                    vL.techni = 1;
                    ...
                    vL.NumberOfTimesExpRepeated = totaldata;
                ...
                save(strcat('D:\Sumedh\Projects\Methods for psychiatric DBS programming\DataA\Ensemble method\',dst,num2str(model),num2str(totaldata),'.mat'),'dat1','-v7.3');
                dat1 = [];
                data1 = [];
            end
            %         seq{test,:} = prb;
