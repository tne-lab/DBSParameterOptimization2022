...
%}
function [dat1] = cm_ts_main_en()%subj1,subj)
    ...
    for dst = ["noalgorithm","UCB"]%,"UCB"]%"greedy","egreedy","UCB","UCBbay","bernoulli","Poisson","Normal","bothNormal"]
        ...
        for test = 1
            ...
            for divisor = 15
                ...
                save(strcat('D:\Sumedh\Projects\Methods for psychiatric DBS programming\DataA\Ensemble method\',dst,num2str(model),'ensemble3days.mat'),'dat1','-v7.3');
                dat1 = [];
                data1 = [];
            end
            %         seq{test,:} = prb;
