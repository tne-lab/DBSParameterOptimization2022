function [disV,conV,bookV,vL,minmax,maxmax] = ts_reset_variables_ensemble(vL,episode)
% new gen and sen
...
if mod(episode-1,ttl/1000) == 0 || episode ==1
    ...
    %disp(episode)
    %disp(strcat(vL.GenName))
    load(strcat(loc,subNumber(vL.geni),"\",vL.GenName));
    % load(strcat(loc2,subNumber(vL.seni),"\",vL.SenName));
    %changing the sensor each iteration
    sid = randi([1 175]);j = randi([1 length(subNumber)]);
    %load("D:\Sumedh\Projects\Methods for psychiatric DBS programming\Data\Sensor\Hand Design\SDMall.mat");
    load(strcat(loc2,subNumber(j),"\",num2str(sid),subNumber(j),"SDMall.mat"))
    vL.SenName =strcat(num2str(sid),subNumber(j),"SDMall.mat");
    vL.seni = j;
